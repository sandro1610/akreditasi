    ...
    <div class="table-responsive py-4">
      <table class="table table-flush" id="data-c7">
        ...
        <tbody>
          <?php
          $sql = "SELECT * FROM c7
                  LEFT JOIN tb_user ON c7.user_id = tb_user.id";
          ...
          ?>
            <tr>
              ...
              <td>
                ...
                |
                <a class="btn btn-sm btn-default" href="javascript:hapusData_c7('<?= $hasil['id']; ?>')">
  ...
  if (in_array($fileActualExt, $allowed)) {
    if ($fileError === 0) {
      if ($fileSize < 100000000) {
        ...
        $sql = mysqli_query($link, "UPDATE `c7` SET `file_c7` = '$fileNewName', `tgl_upload` = '$tgl' WHERE `c7`.`id` = $id");
        if ($sql) {
